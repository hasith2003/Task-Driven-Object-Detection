...
from coco_tasks.graph_networks_improved import GGNN, GGNNBboxNoImg
...
class JointGraphExperiment(object):
    ...
    def get_name(self) -> str:
        string_format = "joint-graph-n:{nname}-d:{dname}-o:{oname}(lr:{lr:f}-wd:{wd:f})".format(
            nname=self.network.__repr__(),
    ...
    def train_n_epochs(self, n: int, overfit: bool = False, lr_scheduler: bool = False):
        ...
        for epoch_number in range(n):
            ...
            for batch in tqdm(train_data_loader):
                ...
                logits = self.network.forward(x, c, d)
    ...
    def do_test(self, test_db: Union[CocoTasksTest, CocoTasksTestGT], task_number: int):
        ...
        data_loader = DataLoader(
            test_db, batch_size=1, num_workers=NUM_WORKERS, collate_fn=test_db.custom_collate
        )
        with torch.no_grad():
...
class JointGraphExperimentBboxNoImg(object):
    """
    For CVPR rebuttal.
    """
    ...
    def get_name(self) -> str:
        string_format = "joint-graph-bbox-no-img-n:{nname}-d:{dname}-o:{oname}(lr:{lr:f}-wd:{wd:f})".format(
            nname=self.network.__repr__(),
            dname=self.dataset.__repr__(),
            oname=self.optimizer.__class__.__name__,
            lr=self.optimizer.defaults["lr"],
        ...
        for epoch_number in range(n):
            if overfit:
                train_data_loader = DataLoader(
                    ...
                    shuffle=False,
                    num_workers=NUM_WORKERS,
                    collate_fn=self.dataset.custom_collate_with_bbox,
                    sampler=sampler,
                )
            else:
                train_data_loader = DataLoader(
            ...
            for batch in tqdm(train_data_loader):
                x, t, m, c, d, bbox = batch  # type: Tensor
                x, t, m, c, d, bbox = (
                    x.to(self.device),
                    t.to(self.device),
                    m.to(self.device),
                ...
                self.optimizer.zero_grad()
                logits = self.network.forward(bbox, c, d)
                loss = self.network.compute_loss(logits, t, m)
    ...
    def do_test(self, test_db: Union[CocoTasksTest, CocoTasksTestGT], task_number: int):
        ...
        data_loader = DataLoader(
            ...
            batch_size=1,
            num_workers=NUM_WORKERS,
            collate_fn=test_db.custom_collate_with_bbox,
        )
        with torch.no_grad():
            for batch in tqdm(data_loader):
                x, c, d, bbox, detections = batch
                x, c, d, bbox = (
                    x.to(self.device),
                    c.to(self.device),
                    d.to(self.device),
                ...
                probabilities = self.network.estimate_probability(bbox, c, d)
                task_probabilities = probabilities[:, task_id].detach().cpu().numpy()
