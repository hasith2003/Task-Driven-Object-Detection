...
class YOLOFeatureExtractor(nn.Module):
    ...
    DEFAULT_HOOK_INDICES = {
        'P3': 15,   # stride-8  output,  ~64  channels
        'P4': 18,   # stride-16 output,  ~128 channels
        'P5': 21,   # stride-32 output,  ~256 channels
    }
    ...
    def _load_yolo(self, weights_path: str) -> nn.Module:
        ...
        try:
            ...
            # .model is the raw nn.Module (DetectionModel)
            backbone = yolo_wrapper.model
            backbone.eval()
