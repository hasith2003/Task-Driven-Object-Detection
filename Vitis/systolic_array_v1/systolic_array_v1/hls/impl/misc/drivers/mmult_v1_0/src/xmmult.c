...
void XMmult_Set_out_r(XMmult *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XMmult_WriteReg(InstancePtr->Control_BaseAddress, XMMULT_CONTROL_ADDR_OUT_R_DATA, (u32)(Data));
    XMmult_WriteReg(InstancePtr->Control_BaseAddress, XMMULT_CONTROL_ADDR_OUT_R_DATA + 4, (u32)(Data >> 32));
}

u64 XMmult_Get_out_r(XMmult *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XMmult_ReadReg(InstancePtr->Control_BaseAddress, XMMULT_CONTROL_ADDR_OUT_R_DATA);
    Data += (u64)XMmult_ReadReg(InstancePtr->Control_BaseAddress, XMMULT_CONTROL_ADDR_OUT_R_DATA + 4) << 32;
    return Data;
}
